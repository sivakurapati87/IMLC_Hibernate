...
public class CommonUtil {
	...
	/**
	 * Get the lookup details values based on parent value
	 * 
	 * @param parent
	 * @return
	 */
	public static List<LookUpDetailJson> getLookupDetailsByParent(Integer parent) {
		List<LookUpDetailJson> jsonList = new ArrayList<LookUpDetailJson>();
		try {
			RestTemplate restTemplate = new RestTemplate();
			ParameterizedTypeReference<List<LookUpDetailJson>> typeRef = new ParameterizedTypeReference<List<LookUpDetailJson>>() {
			};
			ResponseEntity<List<LookUpDetailJson>> result = restTemplate.exchange(Constants.LookUp.GETLOOKUPDETAILSBYPARENT + "/" + parent, HttpMethod.GET, null, typeRef);
			jsonList = result.getBody();
		} catch (Exception e) {
			e.printStackTrace();
			LOG.error(e.getMessage());
	...
	public static Boolean submitGlobalParameter(GlobalParameterJson json) {
		Boolean isSubmitted = Boolean.FALSE;
		try {
			RestTemplate restTemplate = new RestTemplate();
			HttpEntity<GlobalParameterJson> httpEntity = new HttpEntity<GlobalParameterJson>(json);
			ParameterizedTypeReference<Boolean> typeRef = new ParameterizedTypeReference<Boolean>() {
			};
			ResponseEntity<Boolean> result = restTemplate.exchange(Constants.GlobalParameter.Submit, HttpMethod.POST, httpEntity, typeRef);
	...
	public static List<CountryJson> getAllCountries() {
		List<CountryJson> jsonList = new ArrayList<CountryJson>();
		try {
			RestTemplate restTemplate = new RestTemplate();
			ParameterizedTypeReference<List<CountryJson>> typeRef = new ParameterizedTypeReference<List<CountryJson>>() {
			};// return type
			ResponseEntity<List<CountryJson>> result = restTemplate.exchange(Constants.LookUp.GETALLCOUNTRIES, HttpMethod.GET, null, typeRef);
			jsonList = result.getBody();
		} catch (Exception e) {
			e.printStackTrace();
			LOG.error(e.getMessage());
	...
	public static List<CurrencyJson> getAllCurrency() {
		List<CurrencyJson> jsonList = new ArrayList<CurrencyJson>();
		try {
			RestTemplate restTemplate = new RestTemplate();
			ParameterizedTypeReference<List<CurrencyJson>> typeRef = new ParameterizedTypeReference<List<CurrencyJson>>() {
			};// return type
			ResponseEntity<List<CurrencyJson>> result = restTemplate.exchange(Constants.LookUp.GETALLCURRENCY, HttpMethod.GET, null, typeRef);
			jsonList = result.getBody();
		} catch (Exception e) {
			e.printStackTrace();
			LOG.error(e.getMessage());
	...
	public static List<?> findAll(String url) {
		List<?> jsonList = new ArrayList<Object>();
		try {
			RestTemplate restTemplate = new RestTemplate();
			ParameterizedTypeReference<List<?>> typeRef = new ParameterizedTypeReference<List<?>>() {
			};// return type
			ResponseEntity<List<?>> result = restTemplate.exchange(url, HttpMethod.GET, null, typeRef);
			jsonList = result.getBody();
		} catch (Exception e) {
			e.printStackTrace();
			LOG.error(e.getMessage());
